...
$deptStats = $q->getDeptDistribution();
...
<body id="altBody" style="overflow-x: hidden;">
    ...
    <?php foreach ($deptStats as $stat): ?>
        <div style="background: white; padding: 15px; border-radius: 10px; border-left: 5px solid #007bff; box-shadow: 0 2px 5px rgba(0,0,0,0.1); min-width: 150px; flex: 1;">
            <div style="font-size: 0.8rem; color: #666; text-transform: uppercase;"><?= htmlspecialchars($stat['department']) ?></div>
            <div style="font-size: 1.5rem; font-weight: bold; color: #333;"><?= number_format($stat['percentage'], 1) ?>%</div>
            <div style="font-size: 0.75rem; color: #999;"><?= $stat['count'] ?> Total Items</div>
        </div>
    <?php endforeach; ?>
</div>
