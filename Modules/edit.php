...
<form action="../Config/updateAction.php" method="POST" style="width: 100%;">
    ...
    <div class="content">
            ...
            <div class="custom-select" onclick="toggleInventorySelect(this)">
                ...
                <div class="dropdown-menu">
                    <div class="dropdown-item delete-item"
                        onclick="confirmDeleteMaintenance('<?= $data['M_ID'] ?>')">
                ...
                <select name="Dept_ID" class="value">
                    <?php foreach ($departments as $dept): ?>
                        <option value="<?= $dept['D_ID'] ?>"
                            <?= ($data['Dept_ID'] == $dept['D_ID']) ? 'selected' : '' ?>>
        ...
        <div class="form-group">
            <textarea name="description" class="message-box" rows="6"><?= htmlspecialchars($data['desc'] ?? '') ?></textarea>
        </div>
        ...
        <div class="status-section">
            <div class="status-row">
                ...
                <select name="priority"
                    class="badge <?= strtolower($data['Priority'] ?? 'unassigned') ?>"
                    onchange="updateBadgeColor(this)">
                    <?php foreach (['N/A', 'Low', 'Medium', 'High'] as $prio): ?>
                        <option value="<?= $prio ?>"
                            <?= (($data['Priority'] ?? '') == $prio) ? 'selected' : '' ?>>
                            <?= $prio ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="status-row">
                ...
                <select name="status"
                    class="badge <?= strtolower($data['Status'] ?? 'unassigned') ?>"
                    onchange="updateBadgeColor(this)">
                    <?php foreach (['N/A', 'Scheduled', 'Ongoing', 'Resolved'] as $stat): ?>
                        <option value="<?= $stat ?>"
                            <?= (($data['Status'] ?? '') == $stat) ? 'selected' : '' ?>>
                            <?= $stat ?>
                        </option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="content">
            ...
            <div class="info-grid">
                <div>
                    <div class="grid-row">
                        ...
                        <input type="text" name="item_name" class="value" value="<?= htmlspecialchars($data['item_name'] ?? '') ?>">
                    </div>
                    <div class="grid-row">
                        <span class="label">Item Brand:</span>
                        <input type="text" name="item_brand" class="value" value="<?= htmlspecialchars($data['item_brand'] ?? '') ?>">
                    </div>
                    <div class="grid-row">
                        <span class="label">Item Type:</span>
                        <select name="categ_id" class="value">
                            <?php $inv_categories = $q->getInventoryCategories(); 
                            foreach ($inv_categories as $cat): $selected = (($data['categ_ID'] ?? '') == $cat['IC_ID']) ? 'selected' : '';?>
                            
                            <option value="<?= $cat['IC_ID'] ?>" <?= $selected ?>><?= htmlspecialchars($cat['category_name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                ...
                <div>                   
                    <div class="grid-row">
                        ...
                        <input type="text" name="supplier" id="supplier" class="value" value="<?= htmlspecialchars($data['item_supplier'] ?? '') ?>" required>
                    </div>
            ...
            <div class="status-section">
                 <div class="status-row">
                        ...
                        <input type="text" name="serial" class="badge" value="<?= htmlspecialchars($data['Serial_number'] ?? 'N/A') ?>" 
                ...
                <div class="status-row">
                    ...
                    ?>
                    <select name="defects" id="defectsSelect" 
                            class="badge <?= $defectClass ?>" 
        ...
        <div class="content">
            <div class="page-header">
                ...
                <div class="custom-select" onclick="toggleInventorySelect(this)">
                    <i class="fas fa-ellipsis-v"></i>
                    <div class="dropdown-menu">
                        <div class="dropdown-item delete-item" onclick="confirmDelete('<?= htmlspecialchars($data['ticket_num'] ?? '') ?>')">
                            ...
                            <select name="issued_item_id" class="value" style="color: #28a745; font-weight: bold;">
                                ...
                                ?>
                                    <option value="<?= $inv['I_ID'] ?>" <?= $isSelected ?>>
                                        <?= htmlspecialchars($inv['item_name']) ?> (Brand: <?= htmlspecialchars($inv['item_brand']) ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                ...
                <div>
                    ...
                    <div class="grid-row">
                        ...
                        <select name="type" id="type" class="value">
                            ...
                            ?>
                                <option value="<?= $cat['TC_ID'] ?>" <?= $isSelected ?>><?= htmlspecialchars($cat['categ_name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="grid-row">
                        ...
                        <input type="number" name="issued_qty" class="value" 
                            ...
                            value="<?= htmlspecialchars($data['issued_qty'] ?? 0) ?>" min="0" max="200" title="Quantity cannot exceed current stock">
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="form-group">
                <textarea name="description" class="message-box" rows="6"><?= htmlspecialchars($data['T_description'] ?? '') ?></textarea>
            </div>
            ...
            <?php if (isset($role) && $role != 3): ?>
                <div class="status-section">
                    <div class="status-row">
                        <span class="status-label">Priority:</span>
                        <select name="priority" class="badge <?= strtolower($data['Priority'] ?? 'unlabeled') ?>" onchange="updateBadgeColor(this)" required>
                    ...
                    <div class="status-row">
                        <span class="status-label">Status:</span>
                        <select name="status" class="badge <?= strtolower($data['Status'] ?? 'unresolved') ?>" onchange="updateBadgeColor(this)" required>
                    ...
                    <div class="status-row">
                        ...
                        <select name="staff_id" class="badge <?= empty($data['Assigned_To']) ? 'unassigned' : 'assigned' ?>" onchange="updateBadgeColor(this)">
            ...
            <div class="form-footer">
                <div class="actions">
                    <a href="tileView.php?id=<?= $id ?>&mode=ticketing" class="btn cancel">Cancel <i class="fas fa-x"></i></a>
